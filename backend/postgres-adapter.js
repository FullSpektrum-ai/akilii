  ...
  function prepare(query) {
    return {bind(...args) {
      ...
      const exec=async tx=>{
        ...
        if(query==='DELETE FROM profiles WHERE user_id=?'){await tx`delete from email_receipts where user_id=${args[0]}`;await tx`delete from outcomes where user_id=${args[0]}`;await tx`delete from threads where user_id=${args[0]}`;await tx`delete from projects where user_id=${args[0]}`;await tx`delete from workspace_settings where user_id=${args[0]}`;await tx`delete from runs where user_id=${args[0]}`;await tx`delete from connections where user_id=${args[0]}`;}
