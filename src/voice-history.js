...
async function flushVoice(v){if(!v.session_id||v.saving||!v.dirty)return;v.saving=true;v.dirty=false;try{const turns=[...v.turns.values()].filter(t=>t.content).map(t=>({...t}));for(let i=0;i<turns.length;i+=3)await api('voice/transcript','POST',{session_id:v.session_id,turns:turns.slice(i,i+3)});voiceEverSaved=true;await refresh();if(S.data?.profile&&!voiceSession&&S.cid===v.conversation_id)await openChat(v.conversation_id);}catch{v.dirty=true;toast('Voice transcript could not save. Keep this page open and retry from voice mode.');}finally{v.saving=false;if(!v.dirty)pendingVoice.delete(v);voiceSaveStatus();}}
