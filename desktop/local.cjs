...
async function chat(model,messages,signal,onText){const r=await fetch(base+'/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model,messages,stream:true}),signal});if(!r.ok)throw Error('Local model request failed');let pending='';const decoder=new TextDecoder();let done=false;function line(s){if(!s.trim())return;const j=JSON.parse(s);if(j.error)throw Error('Local model reported an error');if(j.message?.content)onText(j.message.content);if(j.done)done=true;}
