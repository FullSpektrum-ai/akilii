...
async function startSharedHost(directory,openExternal=async()=>{},fetchImpl=globalThis.fetch){
 ...
 const server=http.createServer(async(req,res)=>{try{
  ...
  let response;
  if(mode==='local'&&['/api/connections','/api/mcp','/api/runtime'].includes(url.pathname)){
   let result;const b=chunks.length?JSON.parse(Buffer.concat(chunks).toString()):null;
   if(url.pathname==='/api/connections'){
    if(req.method==='POST'){if(b?.server_key!=='akilii-work'||typeof b.enabled!=='boolean'){res.writeHead(400,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Choose the akilii Work integration.'}));return;}localSettings.setWorkTools(b.enabled);}
    result={connections:[{server_key:'akilii-work',status:localSettings.read().workTools?'connected':'revoked',scopes:['work.read']}]};
   }else if(url.pathname==='/api/runtime')result={capabilities:{direct:{available:true,tools:['work_list']},flowstate:{available:false,reason:'FlowState execution is not attached to this local workspace.'},mcp:{available:true,reason:'Internal read-only Work transport'}}};
   else{const base={jsonrpc:'2.0',id:b?.id??null};if(!localSettings.read().workTools)result={...base,error:{code:-32001,message:'Enable akilii Work first.'}};else if(b?.method==='tools/list')result={...base,result:{tools:[{name:'work_list',description:'Read up to five saved local Work plans.',inputSchema:{type:'object',properties:{},additionalProperties:false}}]}};else if(b?.method==='tools/call'&&b.params?.name==='work_list')result={...base,result:{content:[{type:'text',text:JSON.stringify(sql.prepare('SELECT id,title,body,version FROM work_items WHERE user_id=? ORDER BY updated_at DESC LIMIT 5').all('local-device-owner'))}]}};else result={...base,error:{code:-32601,message:'This local tool is unavailable.'}};}
   response=new Response(JSON.stringify(result),{headers:{'Content-Type':'application/json','Cache-Control':'no-store'}});
  }else if(mode==='local'&&['/api/avatar','/api/workspace'].includes(url.pathname)){
  ...
  if((response.headers.get('content-type')||'').includes('text/html')){let html=await response.text();html=html.replace('<script>/* akilii-shared-app */', '<script>if(new URLSearchParams(location.search).get("phase8")!=="demo"){'+fs.readFileSync(path.join(__dirname,'bridge-ui.js'),'utf8')+'}</script><script>/* akilii-shared-app */');response=new Response(html,{status:response.status,headers:response.headers});}
