...
function requireService(services, name, method) {
  ...
  if (!service || typeof service[method] !== 'function') {
    throw new HttpError(503, `${name} capability is unavailable.`, 'CAPABILITY_UNAVAILABLE');
  }
  return service;
}
...
export function createHttpApi({ services = {} } = {}) {
  async function dispatch(request, context = {}) {
    const actor = requireActor(context);
    const url = new URL(request.url);
    const route = matchRoute(request.method, url.pathname);
    if (!route) throw new HttpError(404, 'This API route was not found.', 'NOT_FOUND');
    if (!route.name) throw new HttpError(405, 'This method is not allowed for this route.', 'METHOD_NOT_ALLOWED');

    if (route.name === 'bootstrap') {
      return json(await requireService(services, 'bootstrap', 'load').load({ subjectId: actor.id, user: actor }));
    }
    ...
    if (route.name === 'threads.transition') {
      return json(await requireService(services, 'threads', 'transition').transition({ ...body, id: route.params[0] }));
    }
    ...
    if (route.name === 'work.transition') {
      return json(await requireService(services, 'work', 'transition').transition({ ...body, id: route.params[0] }));
    }
    if (route.name === 'chat.start') {
      const result = await requireService(services, 'chat', 'start').start({ ...body, modality: 'text' });
    ...
    if (route.name === 'voice.start') {
      const result = await requireService(services, 'voice', 'start').start({ ...body, modality: 'voice' });
      return json({
        runId: result.run.runId,
    ...
    throw new HttpError(404, 'This API route was not found.', 'NOT_FOUND');
  }
  ...
  async function handle(request, context = {}) {
    try { return await dispatch(request, context); }
      ...
      const headers = mapped.status === 405 ? { Allow: matchRoute('GET', new URL(request.url).pathname)?.allowed?.join(', ') || '' } : {};
