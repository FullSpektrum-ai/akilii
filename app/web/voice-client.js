...
export function createVoiceClient({
  ...
  if (!api?.createVoiceSession || !api?.saveVoiceTranscript) throw new TypeError('Voice API is required.');
  let active = null;
  ...
  function record(session, role, id, content) {
    if (!id || typeof content !== 'string' || !content.trim()) return;
    const safeId = String(id).replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 100);
    const prior = session.turns.get(safeId);
    const turn = {
      id: safeId,
      role,
  ...
  async function start(input = {}) {
    ...
    if (!navigatorLike?.mediaDevices?.getUserMedia || typeof RTCPeerConnectionImpl !== 'function') {
      throw Object.assign(new Error('Voice is unavailable in this browser.'), { code: 'CAPABILITY_UNAVAILABLE' });
    }

    const stream = await navigatorLike.mediaDevices.getUserMedia({ audio: true });
    const pc = new RTCPeerConnectionImpl();
    const dc = pc.createDataChannel('oai-events');
    const session = { pc, dc, stream, turns: new Map(), nextOrder: 0, audio: null };
    active = session;
  ...
  return Object.freeze({ start, stop, status: () => active ? 'connected' : 'idle' });
}
